...
evaluate_rc_puls:
		...
		lds	YL, neutral_l
		lds	YH, neutral_h
		movw	temp1, _rx_l		; Atomic copy of rc pulse length
		sub	temp1, YL
		sbc	temp2, YH
		brcc	puls_plus
		.if RC_PULS_REVERSE
...
puls_not_zero:
		.if RCP_DEADBAND
		subi	temp1, lo8(RCP_DEADBAND * CPU_MHZ)
		sbci	temp2, hi8(RCP_DEADBAND * CPU_MHZ)
		brmi	puls_zero
		.endif
.endif
	; The following is used by all input modes
rc_do_scale:	ldi	YL, lo8(MIN_DUTY)	; Offset result so that 0 is MIN_DUTY
		ldi	YH, hi8(MIN_DUTY)
		rcall	mul_y_12x34		; Scaled result is now in Y
		cpi	YL, lo8(MAX_POWER)
		ldi	temp1, hi8(MAX_POWER)
		cpc	YH, temp1
		brlo	rc_not_full
		ldi	YL, lo8(MAX_POWER)
		ldi	YH, hi8(MAX_POWER)
rc_not_full:	ldi	temp1, RCP_TOT		; Check _rc_timeout
		cp	_rc_timeout, temp1
		adc	_rc_timeout, ZH		; Increment if not at RCP_TOT
		sts	rc_duty_l, YL
		sts	rc_duty_h, YH
...
.if USE_I2C
evaluate_rc_i2c:
		movw	YL, _rx_l		; Atomic copy of 16-bit input
		cbr	flags1, (1<<EVAL_RC);+(1<<REVERSE)
		.if MOTOR_REVERSE
		sbr	flags1, (1<<REVERSE)
		.endif
	; MK sends one or two bytes, if supported, and if low bits are
	; non-zero. We store the first received byte in _rx_h, second
	; in _rx_l. There are 3 low bits which are stored at the low
	...
	; original version reaches full power at around 245.
		movw	temp1, YL
		ldi	temp3, lo8(0x100 * (POWER_RANGE - MIN_DUTY) / 247)
		ldi	temp4, hi8(0x100 * (POWER_RANGE - MIN_DUTY) / 247)
		rjmp	rc_do_scale		; The rest of the code is common
.endif
;-----bko-----------------------------------------------------------------
.if USE_UART
evaluate_rc_uart:
		mov	YH, _rx_h		; Copy 8-bit input
		cbr	flags1, (1<<EVAL_RC)+(1<<REVERSE)
		.if MOTOR_REVERSE
		sbr	flags1, (1<<REVERSE)
		.endif
	...
	; Scale so that YH == 200 is MAX_POWER.
		movw	temp1, YL
		ldi	temp3, lo8(0x100 * (POWER_RANGE - MIN_DUTY) / 200)
		ldi	temp4, hi8(0x100 * (POWER_RANGE - MIN_DUTY) / 200)
		rjmp	rc_do_scale		; The rest of the code is common
.endif
;-----bko-----------------------------------------------------------------
...
puls_find_multiplicand:
		.if RCP_DEADBAND
		subi	temp3, lo8(RCP_DEADBAND * CPU_MHZ)
		sbci	temp4, hi8(RCP_DEADBAND * CPU_MHZ)
		.endif
		ldi	temp1, lo8(1999)
		ldi	temp2, hi8(1999)
puls_find1:	adiw	temp1, 1
	...
	; Start with negative POWER_RANGE so that 0 is full throttle
		ldi	YL, lo8(MIN_DUTY - POWER_RANGE)
		ldi	YH, hi8(MIN_DUTY - POWER_RANGE)
		rcall	mul_y_12x34
...
.endif
	; Limit maximum RPM (fastest timing)
		cpi	YL, lo8(TIMING_MAX*CPU_MHZ)
		ldi	temp4, hi8(TIMING_MAX*CPU_MHZ)
		cpc	YH, temp4
		ldi	temp4, hh8(TIMING_MAX*CPU_MHZ)
		cpc	temp5, temp4
		brcc	update_timing3
...
update_timing3:
	; Limit minimum RPM (slowest timing)
		ldi	temp4, hh8(TIMING_MIN*CPU_MHZ)
		cp	temp5, temp4
		brcs	update_timing5
		mov	temp5, temp4
		ldi	YH, hi8(TIMING_MIN*CPU_MHZ)
		ldi	YL, lo8(TIMING_MIN*CPU_MHZ)
update_timing5:
.if TIME_ACCUMULATE
		sts	timing_count, ZH
.endif
	...
	; do this with math instead of two constants, but we need a divide.
		...
		ldi	temp1, lo8(MAX_POWER)
		ldi	temp2, hi8(MAX_POWER)
		cpi	YH, hi8(TIMING_RANGE2*CPU_MHZ)
		ldi	temp4, hh8(TIMING_RANGE2*CPU_MHZ)
		cpc	temp5, temp4
		brcs	update_timing6
		ldi	temp1, lo8(PWR_MAX_RPM2)
		ldi	temp2, hi8(PWR_MAX_RPM2)
		cpi	YH, hi8(TIMING_RANGE1*CPU_MHZ)
		ldi	temp4, hh8(TIMING_RANGE1*CPU_MHZ)
		cpc	temp5, temp4
		brcs	update_timing6
		ldi	temp1, lo8(PWR_MAX_RPM1)
		ldi	temp2, hi8(PWR_MAX_RPM1)
...
;-----bko-----------------------------------------------------------------
start_timeout_start:
		ldi	YL, lo8(timeoutSTART*CPU_MHZ)
		ldi	YH, hi8(timeoutSTART*CPU_MHZ)
		ldi	temp1, hh8(timeoutSTART*CPU_MHZ)
		mov	temp5, temp1
		ret
;-----bko-----------------------------------------------------------------
start_timeout:
		...
		sbc	temp5, ZH
		brcs	start_timeout1
		cpi	YL, lo8(timeoutMIN*CPU_MHZ)
		ldi	temp1, hi8(timeoutMIN*CPU_MHZ)
		cpc	YH, temp1
		ldi	temp1, hh8(timeoutMIN*CPU_MHZ)
		cpc	temp5, temp1
		brcc	start_timeout2
...
.if SLOW_THROTTLE
		...
		; be rate-limited.
		ldi	temp1, lo8(PWR_MIN_START)
		ldi	temp2, hi8(PWR_MIN_START)
		cp	YL, temp1
		cpc	YH, temp2
		brcs	set_new_duty12
...
switch_power_off:
		...
		ldi	ZL, pm_lo8(pwm_off)	; Set PWM interrupt vector
		all_pFETs_off temp1
		all_nFETs_off temp1
		ret
;-----bko-----------------------------------------------------------------
control_start:
	...
	; after any jumper change.
		...
		ldi	temp1, hi8(UBRR_VAL)
		out	UBRRH, temp1
		ldi	temp1, lo8(UBRR_VAL)
		out	UBRRL, temp1
...
;		out	TWCR, temp1
		...
		.if SPI_DIR & ((1<<T3)|(1<<T4)|(1<<T5))	; use preset address if T3, T4,
							; or T5 are being used as outputs
		lds	r24, twi_address
		.else
		in	r24, SPI_PORT			; enable internal pull-ups (note
							; that they will be cleared by the
							; motor PWMing, but we don't care)
		ori	r24, (1<<T3)|(1<<T4)|(1<<T5)
		out	SPI_PORT, r24
		ser	r23
wait_for_pullup:
		dec	r23
		brne	wait_for_pullup		
		in	r24, SPI_PIN			; read 3-bit val to add to address
		andi	r24, (1<<T3)|(1<<T4)|(1<<T5)
		.if	T3 > 0
		ori	r24, 1<<(T3-1)
shift_twi_addr:	lsr	r24
		brcc	shift_twi_addr
		.endif
		ldi	r23, I2C_ADDR
		add	r24, r23
		...
		rjmp	i_rc_puls1
i_rc_puls1_notzero:
		lds	temp2, beep_on_nonzeros
		inc	temp2
		sts	beep_on_nonzeros, temp2
		brne	i_rc_puls1
		rcall	beep_f1
i_rc_puls1:	clr	_rc_timeout
		cbr	flags1, (1<<EVAL_RC)+(1<<I2C_MODE)+(1<<UART_MODE)
i_rc_puls2:	wdr
		.ifdef HK_PROGRAM_CARD
		.endif
		sts	got_started, ZH		; zero out
		sbrs	flags1, EVAL_RC		; LAB: loop unless we got TWI data
		rjmp	i_rc_puls2
		rcall	evaluate_rc_init	; LAB: calls evaluate_rc_i2c
		lds	YL, rc_duty_l
		lds	YH, rc_duty_h
		adiw	YL, 0			; Test for zero
						; LAB: ^^ means we must receive a 0 thrust command to boot up
		brne	i_rc_puls1_notzero
		ldi	temp1, 10		; wait for this count of receiving power off
		cp	_rc_timeout, temp1	; LAB: ^^ means wait for this many 0 commands
		brlo	i_rc_puls2
		.if USE_I2C
...
i_rc_puls3:
		...
		rcall	beep_f4			; LAB: this just sounds like a long beep
		rcall	beep_f4
...
init_startup:
		...
		ldi	temp1, 1
		sts	got_started, temp1
.if MOTOR_BRAKE
		nFET_brake temp1
.endif
wait_for_power_on:
		wdr
		sbrs	flags1, EVAL_RC		; LAB: now we wait for a non-zero command,
						; and require the # of recent commands of
						; any value to be >= RCP_TOT - 1
...
FETs_off_wt:	dec	temp1
		...
		ldi	YL, lo8(PWR_MIN_START)	; Start with limited power to
		ldi	YH, hi8(PWR_MIN_START) ; reduce the chance that we
...
.else
		...
		lds	temp1, timing_x
		cpi	temp1, hh8(TIMING_MIN*CPU_MHZ)
		sbrc	flags1, POWER_OFF
		brsh	run_to_brake
.endif
		cp	sys_control_l, ZH
		...
		ldi	temp1, lo8(PWR_MAX_START)
		ldi	temp2, hi8(PWR_MAX_START)
		rjmp	run6_3
...
run6_2:		cbr	flags1, (1<<STARTUP)
		...
		subi	YL, lo8(-SLOW_THROTTLE_STEPS)
		sbci	YH, -1 - hi8(SLOW_THROTTLE_STEPS)
		.else
...
wait_timeout:	sts	goodies, ZH
		...
		ret
;-----bko-----------------------------------------------------------------
wait_for_low:	;T_on	T3
		cbr	flags1, (1<<ACO_EDGE_HIGH)
		rjmp	wait_for_edge
;-----bko-----------------------------------------------------------------
wait_for_high:	;T_on	T3
...
; C callbacks
.global TWIUserSignal
.global TWIUserError
TWIUserSignal:	
		push	ZL
		push	ZH
		ldi	ZL, pm_lo8(TWIUserSignal_jumptable)
		ldi	ZH, pm_hi8(TWIUserSignal_jumptable)
		lsl	r24			; saved by caller
		add	ZL, r24
		adc	ZH, r1 			; r1 is __zero_reg
		ijmp
TWIUserSignal_end:
		pop	ZL
		pop	ZH
...
TWIUserSignal_jumptable:
		sbr	flags1, (1<<EVAL_RC)+(1<<I2C_MODE)	; 0
		rjmp	TWIUserSignal_end
		rcall	beep_f1					; 1
		rjmp	TWIUserSignal_end
		rcall	beep_f2					; 2
		rjmp	TWIUserSignal_end
